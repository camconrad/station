...
/// @title Station Task Management with USDC Compensation - Enterprise Version
/// @notice This contract manages task creation, status updates, and USDC reward distribution for multiple organizations.
...
/// @title Station Enterprise Contract
/// @notice This contract facilitates task management for multiple organizations with admin approval and automated USDC payments upon task approval
contract StationEnterprise {
    ...
    /// @notice Struct to define an organization
    /// @dev Organizations are stored in a mapping with the organization's address
    struct Organization {
        string name; // The name of the organization
        uint256 taskCount; // The total number of tasks created by the organization
    }

    /// @notice Mapping to store tasks with unique task IDs
    mapping(uint256 => Task) public tasks;

    /// @notice Mapping to store organization data
    ...
    /// @notice Event emitted when a new organization is created
    /// @param organizationAddress The address of the organization
    /// @param name The name of the organization
    event OrganizationCreated(address organizationAddress, string name);
    ...
    /// @dev Only the owner can create an organization, ensuring unique organization addresses
    /// @param orgAddress The address of the organization
    /// @param name The name of the organization
    function newOrganization(address orgAddress, string memory name) public onlyOwner {
        require(organizations[orgAddress].taskCount == 0, "Organization already exists");
        organizations[orgAddress] = Organization(name, 0);
        emit OrganizationCreated(orgAddress, name);
    }
